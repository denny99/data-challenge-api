...
exports.usersNetworkIdAnalyzePOST = function (args, res, next) {
    ...
    SocialMediaService.getSocialUserById(args.network.value, args.id.value, req.user, function (re, err) {
        ...
        AuthenticationService.findUser(socialUser.id, undefined, args.network.value, undefined, undefined, function (user, err) {
            if (!err) {
                if (user.share === 1 || req.user.userId === user.userId) {
                    var path = ParamUtil.buildPath(['de', socialUser.title, process.env.adzuna_app_id, process.env.aduna_app_key, process.env.glassdoor_app_id, req.headers['user-agent'], req.headers['HTTP_CLIENT_IP'], process.env.glassdoor_app_key, '1']);
